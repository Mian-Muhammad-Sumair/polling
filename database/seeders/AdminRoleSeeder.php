    ...
    public function run()
    {
        ...
        foreach ($roles as $role => $permissionsNames) {
            if($role=='Owner' || $role=='Administrator'){
                $permissions = Permission::where('guard_name', 'employee')->get()->pluck('id'); // get guard name employee all permissions ids
            }else{
                $permissionsNames=array_merge($permissionsNames,$defaultPermissionsNames);
                $permissions = Permission::whereIn('name', $permissionsNames)->where('guard_name', 'employee')->get()->pluck('id'); // get guard name employee custom permissions ids
            }
